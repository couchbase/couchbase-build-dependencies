...
cp centos-5-x86-64-curl-7.21.4.tar.gz $currentdir/
cd $currentdir
rm -rf /opt/couchbase/*
...
cp centos-5-x86-64-libevent-2.0.11-stable.tar.gz $currentdir/
cd $currentdir
rm -rf /opt/couchbase/*
rm -rf v8
...
tar -zcvf centos-5-x86-64-v8-3.9.7.tar.gz /opt/couchbase/
cp centos-5-x86-64-v8-3.9.7.tar.gz $currentdir/
cd $currentdir
rm -rf /opt/couchbase/*
rm -rf icu4c
...
tar -zcvf centos-5-x86-64-icu4c-894be7.tar.gz /opt/couchbase/
cp centos-5-x86-64-icu4c-894be7.tar.gz $currentdir/
cd $currentdir
rm -rf /opt/couchbase/*
rm -rf gperftool
...
cp centos-5-x86-64-gperftools-2.0.tar.gz $currentdir/
cd $currentdir
rm -rf /opt/couchbase/*
rm -rf snappy
...
make install
tar -zcvf centos-5-x86-64-snappy-894be7.tar.gz /opt/couchbase/
cp centos-5-x86-64-snappy-894be7.tar.gz  $currentdir/
cd $currentdir
rm -rf /opt/couchbase/*
rm -rf otp
rm -rf OTP
...
LD_RUN_PATH=/opt/couchbase/lib make
LD_RUN_PATH=/opt/couchbase/lib make install
tar -zcvf mac-10-7-x86-64-otp-OTP_R14B04.tar.gz /opt/couchbase
cp mac-10-7-x86-64-otp-OTP_R14B04.tar.gz $currentdir/
#build sqlite
cd $currentdir
rm -rf /opt/couchbase/*
rm -rf *sqlite*
...
python2.5 setup.py config -I /opt/couchbase/include -L /opt/couchbase/lib build
cd build/lib*
...
tar -zcvf mac-10-7-x86-64-pysqlite-0ff6e.tar.gz /opt/couchbase
cp mac-10-7-x86-64-pysqlite-0ff6e.tar.gz $currentdir/